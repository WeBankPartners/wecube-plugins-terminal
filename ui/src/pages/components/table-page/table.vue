              ...
              >
                ...
                <span class="" v-if="item.filterable && !item.filterable.multifilter">
                  <Poptip content="" placement="bottom" :transfer="tip_transfer" v-model="visibleFilter">
                    <i
                      ...
                      @click="filterIcon(tableEleIndex)"
                      :class="{ on: getColumnFilter(tableEleIndex)._isFilter === 'Y' }"
                    ></i>
                    <div
                      slot="content"
                    ...
                    >
                      <div class="" style="padding: 10px 16px 4px;">
                        <input
                          ...
                          v-model="filterParam"
                          @click.stop.prevent=""
                          @keyup="searchFiltersContent"
                          placeholder="请搜索"
                          @keyup.8="searchFiltersContent"
                          class="tip-search"
                        />
                      </div>
                      <ul>
                        ...
                        <template v-for="(filter, j) in filterListsBySearch">
                          <li
                            class="filters-li"
                            @click="filterTable(item, tableEleIndex, filter)"
                            :key="j"
                ...
                <span class="" v-if="item.filterable && item.filterable.multifilter">
                  <Poptip content="" placement="bottom" :transfer="tip_transfer" v-model="multiVisibleFilter">
                    ...
                    <div
                      slot="content"
                      v-show="pageConfig.table.tableEle[tableEleIndex]._isShowFilterPop"
                      class="multiFilter"
                    >
                      <!-- <div class="" style="padding: 10px 16px 4px;">
          ...
          <template v-for="(value, tableDataIndex) in table.tableData">
            <tr :key="tableDataIndex">
              ...
              <template v-for="(val, i) in table.tableEle">
                <td class="c-dark" v-if="val.display" :key="i" :class="ellipsis(value, val) ? 'tdoverflow' : ''">
                  <!--ellipsis是否显示省略-->
                  <div class="extendStyle" v-if="table.isExtend && i == firstShow">
                    <a v-show="tableDataIndex != currentActive" @click="loadDetail(value, tableDataIndex)"
                      ><i class="ivu-icon ivu-icon-ios-arrow-forward"></i
                    ></a>
                    <a
                      v-show="tableDataIndex === currentActive"
                      @click="loadDetail(value, tableDataIndex)"
                      class="active"
                      ><i class="ivu-icon ivu-icon-ios-arrow-forward"></i
                    ></a>
                  </div>
                  <!-- 显示自定义tip -->
                  <Tooltip
                    v-if="val.toolTips && renderValue(value, val.toolTips)"
                    :content="value[val.toolTips]"
                    class="cell"
                    :delay="1000"
                    placement="bottom-start"
                  >
                    <a
                      @click="shadow(value, val)"
                      v-if="'shadow' in val"
                      class="extend-shadow poplableSty"
                      :style="val.toolTips.style"
                      >{{ renderValue(value, val) }}</a
                    >
                    <span v-if="!('shadow' in val)" :style="val.toolTips.style" class="poplableSty">{{
                      renderValue(value, val)
                    }}</span>
                    <div slot="content">
                      <!--<div class="Newline">{{renderValue(value, val)}}</div>-->
                      <div class="Newline" v-if="val.toolTips.type === 'text'">
                  ...
                  <Tooltip
                    v-if="ellipsis(value, val) && !val.toolTips"
                    :content="renderValue(value, val)"
                    class="cell"
                    :delay="1000"
                    placement="bottom-start"
                  >
                    <a @click="shadow(value, val)" v-if="'shadow' in val" class="extend-shadow poplableSty">{{
                      renderValue(value, val)
                    }}</a>
                  ...
                  <div
                    class="cell"
                    v-if="
                      !ellipsis(value, val) && (!val.toolTips || (val.toolTips && !renderValue(value, val.toolTips)))
                    "
                    @mouseover="isShowEdit(tableDataIndex, i)"
                  >
                    <!--列表内容有跳转-开始-->
                    <a v-if="'shadow' in val" @click="shadow(value, val)" class="extend-shadow">{{
                      renderValue(value, val)
                    }}</a>
                    <!--列表内容有跳转-结束-->
                    ...
                    <template v-if="!('shadow' in val) && !val.tags">
                      <span v-if="!(isShowEditXX(val, tableDataIndex, i) && isShowEditInput)">{{
                        renderValue(value, val)
                      }}</span>
                      <input
                        type="text"
                        ...
                        v-model="editContent"
                        class="input-style"
                        @keyup.enter="inputBlur(val, value)"
                        @blur.prevent="inputBlur(val, value)"
                      />
                      ...
                      <i
                        v-if="val.editable"
                        @click="showEditInput(value, val, tableDataIndex, i)"
                        class="fa fa-pencil cell-edit"
                        aria-hidden="true"
                      ></i>
                    </template>
                    <!--列表内容无跳转-结束-->
                    <template v-if="val.hasIcon && value[val.hasIcon.judgeflag] ^ val.hasIcon.reverse">
                      <Tooltip placement="bottom">
                        <i
                          class="fa cell-icon"
                          :class="val.hasIcon.icon"
                          @click="goToOpe(val.hasIcon, value)"
                          aria-hidden="true"
              ...
              >
                <div style="width: 151px;padding-left: 8px;height: 21px;">
                  ...
                  <template v-if="table.tableData[tableDataIndex]._operatons.length <= 2">
                    <template v-for="(btn_val, btn_i) in table.tableData[tableDataIndex]._operatons">
                      ...
                      <span
                        class="btn-pipe"
                        :key="btn_i + '1'"
                        v-if="(btn_i != 1) & (table.tableData[tableDataIndex]._operatons.length != 1)"
                      >
                        |
                      </span>
                    </template>
                  ...
                  <template v-else>
                    <span
                      :id="table.tableData[tableDataIndex]._operatons[0].btn_func"
                      ...
                      <div slot="content" class="ui-ul-list" v-if="isShowMoreBtnTip">
                        <ul v-if="showMoreNumber === tableDataIndex">
                          <template v-for="(btn, moreIndex) in table.tableData[tableDataIndex]._operatons">
                            <li class="filters-li" :key="moreIndex" v-if="moreIndex != 0" @click="goToOpe(btn, value)">
                              {{ $t(btn.btn_name) }}
                            </li>
                          </template>
                          <li v-if="isNoActions" style="color: red" @click="noActions">无可用操作</li>
          ...
          <tr v-if="table.tableData.length < 1" class="nodataSty c-dark">
            ...
            <td :colspan="tdNumber" v-if="table.noDataTip">
              <template v-if="typeof table.noDataTip === 'string'">
                <span>{{ $t('table.noDataTip') }},您可以</span
                ><span v-html="table.noDataTip">{{ table.noDataTip }}</span>
              </template>
              <template v-if="typeof table.noDataTip === 'object'">
                <span>{{ $t('table.noDataTip') }},您可以</span
                ><a @click="goToConfig(table.noDataTip)">{{ table.noDataTip.linkword }}</a
                >{{ table.noDataTip.name }}
              </template>
            </td>
          </tr>
        </tbody>
...
export default {
  ...
  methods: {
    ...
    shadow (item, val) {
      ...
      if (val.shadow.query) {
        // 根据key值组装传参对象
        router.query = val.shadow.query
          .map(key => {
            return { [key]: this.$root.$validate.valueFromExpression(item, key) }
          })
          .reduce((result, item) => {
    ...
    doCopy: function (no, eleConfig) {
      ...
      this.$copyText(clipboardMsg).then(
        function () {},
        function () {}
      )
    ...
    isShowEditXX (tableEle, rowNumber, columnNumber) {
      if (
        this.$root.$validate.isEmpty(tableEle.editable) &&
        this.showEditRowNumber === rowNumber &&
        this.showEditColumnNumber === columnNumber
      ) {
        return true
...
.table-template {
  ...
  .ui-table-container::-webkit-scrollbar-thumb {
    /*滚动条里面小方块*/
    border-radius: 6px;
  ...
  .ui-table-container::-webkit-scrollbar-track {
    /*滚动条里面轨道*/
    /*-webkit-box-shadow: inset 0 0 5px rgba(0, 0, 0, 0.2);*/
    border-radius: 0;
    /*background: rgba(0, 0, 0, 0.1);*/
