...
package: image
	...
	cd package && zip -9 $(project_name)-$(version)-$(arch).zip image.tar register.xml init.sql ui.zip
...
ifeq ($(arch),arm64)
	docker buildx build --platform linux/arm64 -t $(project_name):$(version) -f Dockerfile_standalone . --load
else
...
package_standalone: image_standalone
	...
	cd package && zip -9 $(project_name)-$(version)-$(arch).zip image.tar init.sql docker-compose.yml README
